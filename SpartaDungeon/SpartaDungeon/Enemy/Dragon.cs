...
public class Dragon : IEnemy
{
    ...
    public int Attack()
    { 
        Random flag = new Random();         // 20퍼센트의 공격 빗나감, 10퍼센트의 치명타 공격, 일반 공격을 구분하기 위한 랜덤 함수
        int result = flag.Next(0, 100);

        if (80 <= result)       // 몬스터의 공격 빗나감 - 0데미지 반한
        {
            return 0;
        }

        else if (70 <= result && result < 79)       // 몬스터의 치명타 공격 - 기본 damage 함수의 1.2배 데미지 반환
        {
